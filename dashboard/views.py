...
ExamFormClass = modelform_factory(
    Exam,
    fields=['title', 'description', 'grade', 'course', 'week', 'total_mcq_score', 'duration_minutes'],
    widgets={'description': forms.Textarea(attrs={'rows': 3})},
)


@staff_required
def exam_builder(request, pk=None):
    """
    صفحة موحّدة لإنشاء/تعديل امتحان بكل أسئلته واختياراته دفعة واحدة،
    بدل التنقل بين قسم الامتحانات وقسم الأسئلة لكل سؤال على حدة.
    """
    exam = get_object_or_404(Exam, pk=pk) if pk else None

    if request.method == 'POST':
        exam_form = ExamFormClass(request.POST, instance=exam)
        if exam_form.is_valid():
            with transaction.atomic():
                exam = exam_form.save()

                # لو بنعدّل امتحان موجود: نمسح أسئلته القديمة ونعيد إنشاءها
                # من البيانات المُرسلة (أبسط وأوضح للمعلم من محاولة "دمج" التعديلات)
                if pk:
                    exam.questions.all().delete()

                question_count = int(request.POST.get('question_count', 0) or 0)
                for qi in range(question_count):
                    q_text = (request.POST.get(f'question_text_{qi}', '') or '').strip()
                    q_type = request.POST.get(f'question_type_{qi}', 'mcq')
                    q_image = request.FILES.get(f'question_image_{qi}')

                    if not q_text and not q_image:
                        continue  # سؤال فاضي بالكامل، تجاهله

                    question = Question.objects.create(
                        exam=exam, question_type=q_type, text=q_text, image=q_image,
                    )

                    if q_type == 'mcq':
                        choice_count = int(request.POST.get(f'choice_count_{qi}', 0) or 0)
                        correct_idx = request.POST.get(f'choice_correct_{qi}')
                        for ci in range(choice_count):
                            c_text = (request.POST.get(f'choice_text_{qi}_{ci}', '') or '').strip()
                            if not c_text:
                                continue
                            Choice.objects.create(
                                question=question, text=c_text,
                                is_correct=(str(ci) == correct_idx),
                            )

                exam.distribute_mcq_scores()
                messages.success(request, "تم حفظ الامتحان وكل أسئلته بنجاح ✅")
                return redirect('dashboard:list', model_slug='exams')
    else:
        exam_form = ExamFormClass(instance=exam)

    existing_questions = []
    if exam:
        existing_questions = list(
            exam.questions.prefetch_related('choices').all()
        )

    return render(request, 'dashboard/exam_builder.html', {
        'exam_form': exam_form,
        'exam': exam,
        'is_edit': bool(pk),
...
def generic_edit(request, model_slug, pk):
    if model_slug == 'exams':
        return exam_builder(request, pk=pk)
    config = _config_or_404(model_slug)
    model = config['model']
